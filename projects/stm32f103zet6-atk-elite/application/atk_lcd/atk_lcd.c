...
static int lcd_init(void)
{
     ...
     * 这行 printf 语句 !!!!!!!
     */
    LOG_I(DBG_TAG, "lcd id:0x%x", lcddev.id);
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    {
        case 12:
            pfont = (uint8_t *)atk_asc2_1206[chr];  /* 调用1206字体 */
            break;

        case 16:
            pfont = (uint8_t *)atk_asc2_1608[chr];  /* 调用1608字体 */
            break;

        case 24:
            pfont = (uint8_t *)atk_asc2_2412[chr];  /* 调用2412字体 */
            break;

        case 32:
            pfont = (uint8_t *)atk_asc2_3216[chr];  /* 调用3216字体 */
            break;
...
OS_CMPOENT_INIT(lcd_init, OS_INIT_SUBLEVEL_HIGH);
