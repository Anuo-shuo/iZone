...
//extern os_sem_t* mqtt_task1;	//mqtt_task1信号量
os_sem_t* sem_mqtt_task1;	//mqtt_task1信号量
os_sem_t* sem_mqtt_task2;	//mqtt_task2信号量
//os_sem_t* sem_mqtt_task3;	//mqtt_task3信号量
...
//MQTT_TASK1任务配置
#define MQTT_TASK1_TASK_PRIO    5             /* 任务优先级 */
#define MQTT_TASK1_STK_SIZE     512            /* 任务堆栈大小 */
os_task_t *MQTT_TASK1_Handler;                 /* 任务控制块 */
void mqtt_task1(void *parameter); /* 任务函数 */

//MQTT_TASK2任务配置
#define MQTT_TASK2_TASK_PRIO    14            /* 任务优先级 */
#define MQTT_TASK2_STK_SIZE     512           /* 任务堆栈大小 */
os_task_t *MQTT_TASK2_Handler;                 /* 任务控制块 */
void mqtt_task2(void *parameter); /* 任务函数 */
//MQTT_TASK3任务配置
#define MQTT_TASK3_TASK_PRIO    11             /* 任务优先级 */
#define MQTT_TASK3_STK_SIZE     512            /* 任务堆栈大小 */
os_task_t *MQTT_TASK3_Handler;                 /* 任务控制块 */
void mqtt_task3(void *parameter); /* 任务函数 */
...
char is_using_lend=0;
void nfc_task(void *parameter); /* 任务函数 */
/********************************************************************************************************/
/* 构建消息 */
...
static void serial_task(void *parameter)
{
    ...
    while (1)
    {
        ...
        {
            //lcd_show_string(10, 150, 200, 300, 16, (char *)USART2_RX_BUF, BLUE); /* 显示读到的字符串 */
...
//            ++device_state.power;
//            device_state.temperature %= 40;
//            device_state.power %= 99;
				//将数据合并后拷贝到pub_buf中
            //os_snprintf(pub_buf, sizeof(pub_buf), onenet_base_dp_upload_str, id, device_state.temperature, device_state.power);
			/***********************************************/
...
//            lcd_show_string(30, 70, 200, 16, 16, "onenet_disconnect", BLUE);
//            break;
//        }
 ...
 * @retval      无
 */
static void mqtt_task1(void *parameter)
{
	parameter=parameter;
	while(1){
		 if (OS_EOK == os_sem_wait(sem_mqtt_task1, OS_WAIT_FOREVER)){
			 os_kprintf("\r\n MQTT_TASK1 \r\n");
 ...
 * @retval      无
 */
static void mqtt_task2(void *parameter)
{
	parameter=parameter;
	while(1){
 ...
 * @retval      无
 */
static void mqtt_task3(void *parameter)
{
	parameter=parameter;
	while(1){
//		 if (OS_EOK == os_sem_wait(sem_mqtt_task3, OS_WAIT_FOREVER)){
//			 os_kprintf("\r\n MQTT_TASK3 \r\n");
//			 
//		 } 
		...
		switch (cmd_h)
		 ...
		 case 6:case 2:case 3:case 4:{
			...
			// mqtt_task1_num=1;
			  break;}
//		 case 2:{
//			  os_kprintf("Recv submessage cmd: cmd_type=0x2x\r\n");
//			os_sem_post(sem_mqtt_task2);
//			// mqtt_task2_num=1;
//			  break;}
//		 case 3:{
//			  os_kprintf("Recv submessage cmd: cmd_type=0x3x\r\n");
//			 os_sem_post(sem_mqtt_task3);
//			// mqtt_task3_num=1;
//			  break;}
//		 case 4:{
//			  os_kprintf("Recv submessage cmd: cmd_type=0x4x\r\n");
//			 os_sem_post(sem_mqtt_task3);
//			// mqtt_task3_num=1;
//			  break;}
		case 5:{
...
//									uint16_t * e = 0;
//									e = (uint16_t *)TagType;
//									//os_kprintf("Snr Index : %d, type = %x \n", snr * 4 + j, *e);
//									for(int n = 0; n < 16; n++){
//										os_kprintf("%x ", (uint16_t)buf[n]);
//									}	
//									os_kprintf("\n");	
//								}
//							}
...
//	TIM_HandleTypeDef  htim8;
  ...
  /* USER CODE BEGIN 2 */
//HAL_TIM_PWM_Start(&htim8,TIM_CHANNEL_1);//***定时器初始化
	
	//信号量创建
	 sem_ota = os_sem_create("sem_ota", 0,1);
	 sem_lcd = os_sem_create("sem_lcd", 0,1);
	 sem_mqtt_task1= os_sem_create("sem_mqtt_task1", 0,1);//mqtt_task1信号量
    //sem_mqtt_task2= os_sem_create("sem_mqtt_task2", 0,1);//mqtt_task2信号量
    //sem_mqtt_task3= os_sem_create("sem_mqtt_task3", 0,1);//mqtt_task3信号量
    ...
    os_task_startup(LCD_Handler);
	 ...
	 MQTT_TASK1_Handler = os_task_create("mqtt_task1", mqtt_task1, OS_NULL, MQTT_TASK1_STK_SIZE, MQTT_TASK1_TASK_PRIO);      
    OS_ASSERT(MQTT_TASK1_Handler);
    os_task_startup(MQTT_TASK1_Handler);
	 
	 MQTT_TASK2_Handler = os_task_create("mqtt_task2", mqtt_task2, OS_NULL, MQTT_TASK2_STK_SIZE, MQTT_TASK2_TASK_PRIO);      
    OS_ASSERT(MQTT_TASK2_Handler);
    os_task_startup(MQTT_TASK2_Handler);
	 
	 MQTT_TASK3_Handler = os_task_create("mqtt_task3", mqtt_task3, OS_NULL, MQTT_TASK3_STK_SIZE, MQTT_TASK3_TASK_PRIO);      
    OS_ASSERT(MQTT_TASK3_Handler);
    os_task_startup(MQTT_TASK3_Handler);
